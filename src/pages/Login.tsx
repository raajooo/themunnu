...
export default function Login({ user }: LoginProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || "Login failed. Please check your credentials.", { id: loginToast });
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <p className="text-center text-xs font-bold text-gray-400 uppercase tracking-widest mt-4">
            Don't have an account? <Link to="/register" state={{ from }} className="text-black dark:text-white underline">Register</Link>
          </p>
        </form>
